...
- hero editorial con retorno al blog, jerarquia `Category -> Angle -> Domain`, titulo, fecha, tiempo de lectura e imagen principal en panel controlado
- cuerpo editorial estructurado y escaneable, no presentado como una sola masa encapsulada
- `Key idea` visible derivada de `excerpt` como primer bloque del area de lectura, antes del cuerpo del articulo
- rail interno `On this page` derivado de headings reales del articulo para navegar sus secciones principales y alineado con el inicio del area de lectura en desktop
...
- `category` es la unica taxonomy del blog con ruta publica propia
- `angle` y `domain` no generan routes, archives ni indices dedicados
